...
class FaceModel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Sequential(
            torch.nn.Conv2d(3, 16, 3, padding=1),  # 3*28*28 -> 16*28*28
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(2, 2)  # 16*28*28 -> 16*14*14
        )
        self.conv2 = torch.nn.Sequential(
            torch.nn.Conv2d(16, 32, 3, padding=1),  # 16*14*14 -> 32*14*14
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(2, 2)  # 32*14*14 -> 32*7*7
        )
        self.conv3 = torch.nn.Sequential(
            torch.nn.Conv2d(32, 64, 3, padding=1),  # 32*7*7 -> 64*7*7
            torch.nn.ReLU(),
            torch.nn.AvgPool2d(7)  # 64*7*7 -> 64*1*1
        )
...
class Client(ClientBase):
    def __init__(self, client_name, train_loader, test_loader, start_epoch=0, checkpoint_path='./', device='cpu',
        ...
        self.model = FaceModel().to(device)
    ...
    def test(self, save_best=False):
        ...
        print('{} Test  | Epoch={:d} | Loss={:.4f} | Acc={:.4f}'.format(self.name, self.cur_epoch - 1,

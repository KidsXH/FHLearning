...
class Client(ClientBase):
    def __init__(self, client_name, train_loader, test_loader, start_epoch=0, checkpoint_path='./', device='cpu',
        ...
        self.model = NutritionModel().to(device)
    ...
    def run(self, n_epochs, save_last=False):
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, (inputs, labels) in enumerate(train_loader):
                ...
                correct_1 += torch.sum((pred == labels.data) & (labels.data == 1)).item()
            ...
            print('{} Train | Epoch={:d} | Loss={:.4f} | Acc={:.4f} | {}'.format(self.name, self.cur_epoch,
                                                                                 epoch_loss, epoch_acc, correct_1))
    ...
    def test(self, save_best=False):
        ...
        with torch.no_grad():
            for (inputs, labels) in test_loader:
                ...
                total += inputs.size(0)
                correct += torch.sum(pred == labels.data).item()
                correct_0 += torch.sum((pred == labels.data) & (labels.data == 0)).item()
                correct_1 += torch.sum((pred == labels.data) & (labels.data == 1)).item()
                running_loss += loss.item() * inputs.size(0)
        ...
        print(
            '{} Test  | Epoch={:d} | Loss={:.4f} | Acc={:.4f} | Correct01={}/{}'.format(self.name, self.cur_epoch - 1,
